...
exports.build = (startDate, debts, payments) => {
    ...
    const sumPayments = _.map(payments, (payment) => payment.value).reduce((a, b) => a + b, 0); // TODO break up payments by user
    ...
    for (let i = 0; i < debts.length; i++) {
        const debtSpecificSchedule = this.buildForDebt(debts[i], rank, date, sumPayments);

        _.each(debtSpecificSchedule, scheduleItem => paymentSchedule.push(scheduleItem));
...
exports.buildForDebt = (debt, startRank, startDate, paymentAmount) => {
    ...
    for (let i = 0; i < numberOfPayments; i++) {
        ...
        if (i == 0)
            ...
            nextDate = previousDate.plusDays(14);
